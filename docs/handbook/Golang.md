...
<p>然后就要将GC的执行流程了，一共分为5个阶段，清除终止阶段，标记阶段，标记终止阶段，清理阶段</p>
<p>清除终止阶段会开启STW，然后会处理还没被清理的内存单元(强制执行GC可能会出现该情况)</p>
<p>标记阶段会开启写屏障，启动用户协助程序并将所有根对象入队，然后会关闭STW，此时有新创建的对象都会被标记为黑色。然后开始扫描根对象，当扫描到某个goroutine栈时，会暂停该goroutine，这里会优先扫描栈。当扫描完成后将会进入标记终止阶段</p>
